  ...
  try {
    globalThis.fetch = async (input) => {
      ...
      return Response.json({
        results: [{ id: Number(page) }],
        next: page === "1" ? "http://api:8000/api/indents/?page=2" : null,
      });
    };
...
test("reports expose the next page instead of hiding rows after the first 50", async ({ page }) => {
  await page.route("**/api/**", async (route) => {
    ...
    if (url.pathname === "/api/reports/approved-unpaid/")
      json = {
        slug: "approved-unpaid",
        name: "Approved but unpaid",
        count: 51,
        columns: ["trip"],
        rows: [{ trip: url.searchParams.get("page") === "2" ? "LAST-TRIP" : "FIRST-TRIP" }],
        totals: {},
      };
