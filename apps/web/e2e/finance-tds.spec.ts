...
test("finance can record TDS-only allocations with zero net cash", async ({ page }) => {
  ...
  await page.route("**/api/**", async (route) => {
    ...
    if (path === "/api/finance/pending/") json = [{
      vendor_id: 1, vendor_name: "Test vendor", vendor_code: "V1", vendor_documents: [], total_net: "0.00",
      bank_accounts: [{ id: 1, active: true, bank_name: "Test bank", masked_account_number: "****1234", ifsc_code: "TEST0000001" }],
      items: [{ approval_item_id: 1, approval_id: 1, approval_no: "PA-1", trip_no: "TEST-TRIP",
        remaining_gross: "25.00", remaining_tds: "25.00", remaining_net: "0.00", driver_documents: [], trip_documents: [] }],
    }];
  ...
  expect(requests[0].allocations).toEqual([{ approval_item_id: 1, gross_amount_allocated: "25.00", tds_allocated: "25.00", net_cash_allocated: "0.00" }]);
});
