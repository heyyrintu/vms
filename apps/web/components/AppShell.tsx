  ...
  },
  {
    ...
    links: [
      { label: "Pending payments", href: "/finance", icon: "₹", roles: ["OPERATIONS", ...FINANCE_VIEW] },
      {
        ...
        href: "/payments",
        icon: "▤",
        roles: ["OPERATIONS", "TRANSPORTER", ...FINANCE_VIEW],
      },
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const initials =
    `${me.data?.first_name?.[0] ?? ""}${me.data?.last_name?.[0] ?? ""}` || me.data?.username?.slice(0, 2).toUpperCase();
    ...
    <div className="app-shell">
      <aside className="sidebar">
        <div className="brand brand-lockup">
          <BrandLogo priority />
          <small>Transport operations control</small>
        </div>
      ...
      <main className="main">
        <header className="topbar">
          ...
          <div className="user-pill">
            ...
            <div>
              <strong>{me.data?.first_name || me.data?.username}</strong>
              <br />
      ...
      <nav className="mobile-nav">
        {visibleGroups
          ...
          .filter((link) => link.href in MOBILE_LABELS)
          .map((link) => (
            <Link href={link.href} key={link.href}>
              {MOBILE_LABELS[link.href]}
            </Link>
          ))}
      </nav>
    </div>
