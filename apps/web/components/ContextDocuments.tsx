...
export function ContextDocuments({ objectType, objectId }: { objectType: "approval" | "payment"; objectId: string }) {
  const query = useQuery({
    ...
    queryFn: () => api<Paginated<Document>>(`/documents/?object_type=${objectType}&object_id=${objectId}`),
  });
  ...
  const upload = async (event: FormEvent) => {
    ...
    body.append("kind", objectType === "payment" ? "PAYMENT_PROOF" : "OTHER");
  ...
  return (
    <section className="panel">
      ...
      <form className="panel-body" onSubmit={upload}>
        <div className="field">
        ...
        <button className="button small" style={{ marginTop: 8 }}>
          Upload
        </button>
      </form>
    </section>
