...
export default function ApprovalDetailPage() {
  ...
  const decision = useMutation({
    mutationFn: (value: string) =>
      api<Approval>(`/approval-batches/${id}/decide/`, {
        method: "POST",
        body: JSON.stringify({ decision: value, item_ids: selected.length ? selected : undefined, comment: reason }),
      }),
    onSuccess: (data) => {
      queryClient.setQueryData(["approval", id], data);
      setSelected([]);
  ...
  return (
    <>
      <PageHeader
        title={b.approval_no}
        description={`${b.client_name} · ${b.purpose.replaceAll("_", " ")} · Revision ${b.revision_no}`}
      >
        <StatusBadge value={b.status} />
      </PageHeader>
      {decision.error && <ErrorNotice error={decision.error} />}
      {submission.error && <ErrorNotice error={submission.error} />}
      {b.status === "DRAFT" && (me.data?.id === b.requested_by || me.data?.role === "ADMIN") && (
        <div className="notice">
          This draft has not been sent for approval. Review the saved amounts below, then submit it.
          <div className="actions">
            <button className="button primary" disabled={submission.isPending} onClick={() => submission.mutate()}>
              {submission.isPending ? "Submitting…" : "Submit draft approval"}
            </button>
          </div>
        </div>
      )}
      <div className="summary-strip">
        ...
        <div>
          ...
          <strong>
            <Money value={b.gross_requested} />
          </strong>
        </div>
        <div>
          <span>TDS withholding</span>
          <strong>
            <Money value={b.tds_requested} />
          </strong>
        </div>
        <div>
          <span>Net cash request</span>
          <strong>
            <Money value={b.net_requested} />
          </strong>
        </div>
      </div>
      <section className="panel">
        <div className="panel-head">
          <h2>{b.approval_rule_snapshot.rule_name || "Approval stages"}</h2>
          <span className="eyebrow">Current stage {b.current_stage}</span>
        </div>
        <div className="table-wrap">
          <table>
            ...
            <tbody>
              {b.stage_decisions.map((row) => (
                <tr key={row.id}>
                  <td>{row.sequence}</td>
                  <td>{row.label}</td>
                  <td>{row.role}</td>
                  <td>
                    <StatusBadge value={row.status} /> {row.decided_by_name}
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        {b.revision_diff.length > 0 && (
          <div className="panel-body notice">
            <strong>Revision changes</strong>
            {b.revision_diff.map((row, index) => (
              <div key={index}>
      ...
      <div className="split">
        <div>
          <section className="panel">
            <div className="panel-head">
              <h2>Trip lines</h2>
              <span className="muted">Select lines for a partial final-stage decision</span>
            </div>
            <div className="table-wrap">
              <table>
                <thead>
                    ...
                    <tr key={item.id}>
                      <td>
                        <input
                          ...
                          onChange={(e) =>
                            setSelected((old) =>
                              e.target.checked ? [...old, item.id] : old.filter((value) => value !== item.id),
                            )
                          }
                        />
                      </td>
                      ...
                      <td className="money">
                        <Money value={item.gross_requested} />
                      </td>
                      <td className="money">
                        <Money value={item.tds_this_request} />
                      </td>
                      <td className="money">
                        <strong>
                          <Money value={item.net_requested} />
                        </strong>
                      </td>
                      <td>
          ...
          {canDecide && ["PENDING", "PARTIALLY_APPROVED"].includes(b.status) && (
            <section className="panel">
              <div className="panel-head">
                <h2>{stage?.label || "Approval decision"}</h2>
              </div>
              <div className="panel-body">
                <div className="field">
                  <label>Reason / approver note (required for reject or send back)</label>
                  <textarea
                    className="input"
                    value={reason}
                ...
                <div className="actions" style={{ marginTop: 12 }}>
                  ...
                  >
                    Approve {selected.length ? "selected" : "stage"}
                  </button>
                  <button
                    className="button"
                    onClick={() => decision.mutate("SEND_BACK")}
            ...
            <div className="timeline panel-body">
              {b.actions.map((action) => (
                <div className="timeline-item" key={action.id}>
                  <strong>
                    {action.action.replaceAll("_", " ")} · {action.actor_name}
                  </strong>
                  <small>
                    <DateText value={action.created_at} />
                  </small>
