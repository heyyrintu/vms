...
export default function TripLedgerPage() {
  ...
  return (
    <>
      <PageHeader
        title={`Trip ledger · ${row.trip_no}`}
        description={`${row.vendor} · Created ${new Date(row.created_at).toLocaleString("en-IN")} · Updated ${new Date(row.updated_at).toLocaleString("en-IN")}`}
      />
      <div className="cards">
        {[
          ["Full freight (100%)", row.freight_100],
          [`Advance (${row.advance_percent}%)`, row.advance_amount],
          ["Freight after advance", row.freight_balance_after_advance],
          ["Total vendor liability", row.total_vendor_liability],
          ["Approved gross", row.approved_gross],
          ["Cash paid", row.cash_paid],
          ["TDS deducted", row.tds_deducted],
          ["Remaining to pay", row.remaining_to_pay],
        ].map(([label, value]) => (
      ...
      <section className="panel">
        ...
        {!row.events.length ? (
          <Empty message="No payments have been posted for this trip." />
        ) : (
