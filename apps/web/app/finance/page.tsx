...
function VendorPaymentGroup({ group, refresh }: { group: PendingGroup; refresh: () => void }) {
  ...
  const totals = chosen.reduce(
    (sum, item) => {
      ...
      return {
        gross: sum.gross + Number(allocation.tds || 0) + Number(allocation.net || 0),
        tds: sum.tds + Number(allocation.tds || 0),
        net: sum.net + Number(allocation.net || 0),
      };
    },
    { gross: 0, tds: 0, net: 0 },
  );
  const allocationError = chosen.find((item) => {
    ...
    const tds = Number(allocation.tds);
    const net = Number(allocation.net);
    const gross = tds + net;
    return (
      !Number.isFinite(tds) ||
      !Number.isFinite(net) ||
      ...
      tds < 0 ||
      net < 0 ||
      tds > Number(item.remaining_tds) ||
      net > Number(item.remaining_net) ||
      gross > Number(item.remaining_gross) + 0.005
    );
  });

  const toggleItem = (item: PendingGroup["items"][number], checked: boolean) => {
    setSelected((old) =>
      checked ? [...old, item.approval_item_id] : old.filter((id) => id !== item.approval_item_id),
    );
    if (checked)
      setAllocations((old) => ({
        ...old,
        [item.approval_item_id]: { tds: item.remaining_tds, net: item.remaining_net },
      }));
  };
        ...
        body: JSON.stringify({
          ...
          allocations: chosen.map((item) => ({
            ...
            gross_amount_allocated: (
              Number(allocations[item.approval_item_id]?.tds ?? item.remaining_tds) +
              Number(allocations[item.approval_item_id]?.net ?? item.remaining_net)
            ).toFixed(2),
    ...
    <section className="panel">
      <div className="panel-head">
        <div>
          ...
          <span className="muted">
            {group.vendor_code} · {group.vendor_legal_name} · Cash <Money value={group.total_net} /> · TDS{" "}
            <Money value={group.items.reduce((sum, item) => sum + Number(item.remaining_tds), 0)} /> pending
          </span>
        </div>
        <span className="badge blue">{group.items.length} approved lines</span>
      </div>
      {Boolean(error) && (
      ...
      <div className="panel-body split">
        ...
        <div>
          ...
          {group.bank_accounts.length === 0 ? (
            <div className="notice error">
              No bank account is available. Add and verify a vendor account before paying.
            </div>
          ) : (
            <div className="table-wrap">
              <table>
                ...
                <tbody>
                  {group.bank_accounts.map((account) => (
                    <tr key={account.id}>
                      <td>
                        <strong>{account.bank_name}</strong>
                        <div className="muted">{account.account_holder}</div>
                      </td>
        ...
        <table>
          <thead>
            <tr>
              ...
              <th className="money">Due / TDS / cash</th>
            </tr>
          </thead>
          <tbody>
            {group.items.map((item) => (
              <tr key={item.approval_item_id}>
                <td>
                  <input
                    type="checkbox"
                    className="checkbox"
                    checked={selected.includes(item.approval_item_id)}
                    onChange={(event) => toggleItem(item, event.target.checked)}
                  />
                </td>
                <td>
                  <strong>{item.trip_no}</strong>
                  <div>{item.approval_no}</div>
                  <div className="muted">
                    Approved <DateText value={item.approved_at} />
                  </div>
                </td>
                <td>
                  {item.route}
                  <div className="muted">
                    {item.client_name} · {item.vehicle_no} ({item.vehicle_type})
                  </div>
                  <div className="muted">
                ...
                <td className="money">
                  <Money value={item.advance_amount} />
                  <div className="muted">{item.advance_percent}%</div>
                </td>
                <td className="money">
                  <Money value={item.freight_balance_after_advance} />
                </td>
                <td className="money">
                  <strong>
                    <Money value={item.remaining_gross} />
                  </strong>
                  <div className="muted">
                    TDS <Money value={item.remaining_tds} /> · Cash <Money value={item.remaining_net} />
                  </div>
                </td>
              </tr>
            ))}
      ...
      {selected.length > 0 && (
        <div className="panel-body" style={{ background: "#fff8f5", borderTop: "1px solid var(--line)" }}>
          <h3>Allocate this payment</h3>
          <p className="muted">
            Enter the cash and TDS being recorded now. The unpaid balance stays in this queue for the next payment.
          </p>
          <div className="table-wrap">
            <table>
              <thead>
                <tr>
                  <th>Trip</th>
                  <th className="money">Maximum due</th>
                  <th className="money">TDS now</th>
                  <th className="money">Cash now</th>
                  <th className="money">Gross now</th>
                </tr>
              </thead>
              <tbody>
                {chosen.map((item) => {
                  const allocation = allocations[item.approval_item_id] ?? {
                    tds: item.remaining_tds,
                    net: item.remaining_net,
                  };
                  return (
                    <tr key={item.approval_item_id}>
                      <td>
                        <strong>{item.trip_no}</strong>
                        <div className="muted">{item.approval_no}</div>
                      </td>
                      <td className="money">
                        <Money value={item.remaining_gross} />
                      </td>
                      <td>
                        <input
                          aria-label={`TDS for ${item.trip_no}`}
                          className="input"
                          type="number"
                          min="0"
                          max={item.remaining_tds}
                          step="0.01"
                          value={allocation.tds}
                          onChange={(e) =>
                            setAllocations((old) => ({
                              ...old,
                              [item.approval_item_id]: { ...allocation, tds: e.target.value },
                            }))
                          }
                        />
                      </td>
                      <td>
                        <input
                          aria-label={`Cash for ${item.trip_no}`}
                          className="input"
                          type="number"
                          min="0"
                          max={item.remaining_net}
                          step="0.01"
                          value={allocation.net}
                          onChange={(e) =>
                            setAllocations((old) => ({
                              ...old,
                              [item.approval_item_id]: { ...allocation, net: e.target.value },
                            }))
                          }
                        />
                      </td>
                      <td className="money">
                        <Money value={Number(allocation.tds || 0) + Number(allocation.net || 0)} />
                      </td>
                    </tr>
                  );
                })}
          ...
          {allocationError && (
            <div className="notice error">
              Allocation for {allocationError.trip_no} must be positive and cannot exceed its remaining gross, TDS, or
              cash balances.
            </div>
          )}
          <div className="form-grid">
                ...
                {group.bank_accounts
                  ...
                  .map((row) => (
                    <option value={row.id} key={row.id}>
                      {row.bank_name} · {row.masked_account_number} · {row.ifsc_code}
                    </option>
                  ))}
              </select>
            </div>
            <div className="field span-2">
              ...
              <input
                ...
                placeholder="Enter confirmed reference"
              />
            </div>
            <div className="field span-2">
          ...
          <div className="actions" style={{ justifyContent: "flex-end", marginTop: 12 }}>
            <span className="muted">
              Gross <Money value={totals.gross} /> · TDS <Money value={totals.tds} /> · Net cash{" "}
              <Money value={totals.net} />
            </span>
            <button
              className="button primary"
              disabled={!utr.trim() || !bankAccount || busy || Boolean(allocationError) || totals.gross <= 0}
              onClick={pay}
            >
              {busy ? "Recording…" : "Record paid transaction"}
            </button>
          </div>
        </div>
      )}
...
export default function FinanceQueuePage() {
  ...
  return (
    <>
      <PageHeader
        title="Finance review & pending payments"
        description="Approved payment requests with full trip, KYC, bank and liability evidence in one place."
      />
      <div className="notice">
        Review vendor/driver identity documents, the masked bank account and cancelled cheque, then select approved
        lines to pay. Full account numbers are never shown.
      </div>
      {queue.isPending ? (
        <Loading />
      ) : queue.error ? (
        ...
        <div className="panel">
          <Empty message="No approved unpaid items are waiting for finance." />
        </div>
      ) : (
        queue.data!.map((group) => (
          <VendorPaymentGroup group={group} key={group.vendor_id} refresh={() => queue.refetch()} />
        ))
      )}
    </>
  );
