...
async function uploadDocument(file: File, kind: string, objectType: string, objectId: number) {
  ...
  body.append("object_id", String(objectId));
...
export default function VendorListPage() {
  ...
  const vendors = useQuery({
    ...
    queryFn: () =>
      api<Paginated<Vendor>>(
        `/vendors/?page=${page}&page_size=${pageSize}&search=${encodeURIComponent(search)}&status=${status}`,
      ),
  });
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      const vendor = await api<Vendor>(editingId ? `/vendors/${editingId}/` : "/vendors/", {
      ...
      if (!editingId) {
        ...
        const bank = await api<{ id: number }>("/vendor-bank-accounts/", {
          method: "POST",
          body: JSON.stringify({ vendor: vendor.id, bank_name, account_holder, account_number, ifsc_code }),
        });
  ...
  return (
    <>
      <PageHeader
        title="Transporters & vendors"
        description="Master data, verified identity documents and protected bank details."
      >
        <button
      ...
      {adding && (
        <section className="panel">
          <div className="panel-head">
            <h2>{editingId ? "Edit transporter" : "Create transporter"}</h2>
          </div>
          <form className="panel-body" onSubmit={submit}>
            ...
            {!editingId && (
              <>
                <h3 style={{ marginTop: 24 }}>Identity verification (optional)</h3>
                <div className="form-grid two" key={`kyc-${fileKey}`}>
                  <div className="field">
                    <label htmlFor="vendor-aadhaar">Aadhaar document</label>
                    <input
                      id="vendor-aadhaar"
                      type="file"
                      className="input"
                  ...
                  <div className="field">
                    <label htmlFor="vendor-pan">PAN document</label>
                    <input
                      id="vendor-pan"
                      type="file"
                      className="input"
                ...
                <div className="form-grid two">
                  ...
                  <div className="field span-2" key={`cheque-${fileKey}`}>
                    <label htmlFor="vendor-cancelled-cheque">Cancelled cheque proof</label>
                    <input
                      id="vendor-cancelled-cheque"
                      required
                      type="file"
                      className="input"
                      accept=".pdf,image/jpeg,image/png"
                ...
                <div className="muted" style={{ marginTop: 10 }}>
                  Account numbers are encrypted; only the last four digits are shown after saving. Files are
                  signature-checked and malware-scanned when configured.
                </div>
              </>
            )}
            <div className="actions" style={{ justifyContent: "flex-end", marginTop: 13 }}>
              <button
                type="button"
                className="button"
                onClick={() => {
              ...
              <button disabled={busy} className="button primary">
                {busy ? "Saving…" : editingId ? "Save changes" : "Create vendor"}
              </button>
            </div>
          </form>
        </section>
      ...
      <section className="panel">
        <div className="panel-body">
          <div className="filters">
            <div className="field span-2">
              <label>Search vendor, code, phone or email</label>
              <input
                className="input"
                value={search}
                onChange={(e) => {
        ...
        {vendors.isPending ? (
          ...
          <ErrorNotice error={vendors.error} />
        ) : rows.length === 0 ? (
          <Empty />
        ) : (
              ...
              <tbody>
                {rows.map((vendor) => (
                  <tr key={vendor.id}>
                    ...
                    <td>
                      <DetailLink href={`/vendors/${vendor.id}`}>{vendor.display_name}</DetailLink>
                      <div className="muted">{vendor.legal_name}</div>
                    </td>
                    <td>{vendor.primary_phone || "—"}</td>
                    <td>{vendor.email || "—"}</td>
                    <td>{vendor.vehicles_count}</td>
                    <td>
                      <StatusBadge value={vendor.status} />
                      <div className="actions">
                        <button className="button small" onClick={() => editVendor(vendor)}>
                          Edit
                        </button>
                        <button className="button small" onClick={() => toggleVendor(vendor)}>
        ...
        {vendors.data && <Pagination count={vendors.data.count} page={page} pageSize={pageSize} onPage={setPage} />}
      </section>
    </>
  );
}
