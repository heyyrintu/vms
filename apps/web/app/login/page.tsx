...
export default function LoginPage() {
  ...
  const requestReset = async (event: FormEvent) => {
    ...
    setError("");
    try {
      const result = await api<{ status: string }>("/auth/password/reset/", {
        method: "POST",
  ...
  const confirmReset = async (event: FormEvent) => {
    ...
    try {
      await api("/auth/password/reset/confirm/", {
        method: "POST",
        body: JSON.stringify({ ...resetCredentials, new_password: password }),
      });
        ...
        {resetMode ? (
          <form className="login-form" onSubmit={resetCredentials.token ? confirmReset : requestReset}>
            ...
            <button className="button primary" disabled={busy}>
              {busy ? "Working…" : resetCredentials.token ? "Reset password" : "Send reset link"}
            </button>
            <button type="button" className="button" onClick={() => setResetMode(false)}>
              Back to sign in
            </button>
          </form>
        ) : (
          <form className="login-form" onSubmit={submit}>
            ...
            <button type="button" className="button" onClick={() => setResetMode(true)}>
              Forgot password?
            </button>
